...
export default function CourseFilter({ onSearchSumbit, onFilterSelect }) {
  ...
  return (
    <div className="flex flex-col md:flex-row items-center my-4">
      <div className="flex mr-2 relative rounded-md">
        ...
        />
        <Button onClick={() => onSearchSumbit(searchText)}>Search</Button>
      </div>
      <div className="relative text-gray-700">
        ...
        >
          {OPTIONS.map((optionLabel) => (
            <option key={optionLabel}>{optionLabel}</option>
          ))}
        </select>
